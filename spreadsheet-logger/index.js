...
const writeTitles = (auth) => {
  ...
  sheets.spreadsheets.values.update({
    ...
    resource: {
        values: [
          ['Device', 'Datetime', 'Theta', 'Phi', 'Illuminum', 'Battery', 'RSSI']
        ],
    },
  }, (err, res) => {
    if (err) {
...
//to avoid acquiring multiple data for a same timestamp
var phi_prev = 0;
...
const discovered = (peripheral) => {
    ...
    if (String(device.name).match(/^Leaf_[A-Z]$/) != null){
        ...
        // Decode sensors data
        let theta = ((device.data[0] << 8) + device.data[1]) / 256;
        let phi = ((device.data[2] << 8) + device.data[3]) / 256;
        let light = ((device.data[4] << 8) + device.data[5]);
        let battery = ((device.data[6] << 8) + device.data[7]) / 256;
    
        if (phi_prev != phi){
            let values = [
              [device.name, dt_s, theta, phi, light, battery, device.rssi]
            ];
